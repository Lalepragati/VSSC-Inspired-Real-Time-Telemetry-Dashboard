...
class RocketSimulator:
    ...
    def step(self, dt: float) -> Telemetry:
        # Powered ascent until fuel depletion.
        powered = self.fuel > 0.0

        if powered:
            # Thrust tapers as fuel burns to emulate engine performance drop.
        ...
        # Basic quadratic drag proportional to v*|v|.
        drag_accel = 0.00006 * self.velocity * abs(self.velocity)

        # Net acceleration: up is positive.
        acceleration = thrust_accel - GRAVITY - drag_accel
        self.velocity += acceleration * dt
        self.altitude += self.velocity * dt

        # Ground clamp and bounce suppression during descent end.
        ...
        # Attitude profile with small damped oscillations + noise.
...
async def telemetry_ws(websocket: WebSocket):
    ...
    try:
        while True:
            # Consume frames so disconnects are detected quickly.
